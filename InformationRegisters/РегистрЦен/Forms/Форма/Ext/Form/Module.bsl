Процедура ОбновитьТблОбр()
	...
	|               ВЫБРАТЬ DISTINCT
	               ...
	               |	Рег.ОписаниеПараметров,
	               |	Рег.НаименованиеДляРеестра,
	               ...
	               |		ПО    ро_Оборудование.Ссылка = роЗаказНаряд.Оборудование
				   |          или ро_Оборудование.ссылка = роЗаказНаряд.ТипДопОбр
				   |          или ро_Оборудование.ссылка = роЗаказНаряд.ТипДопОбр1
				   |
				   |
				   |    LEFT OUTER JOIN РегистрСведений.РегистрЦен Рег ON Рег.Контрагент   = роЗаказНаряд.Контрагент
				   |                                                    и Рег.Оборудование = ро_Оборудование.ссылка
				   |
...
Процедура тблОбрЦенаПриИзмененииНаСервере(КА,Обр,Цена,ОписаниеПараметров,НаименованиеДляРеестра)
	
	Зап = РегистрыСведений.РегистрЦен.СоздатьМенеджерЗаписи();
	Зап.Контрагент = КА;
	Зап.Оборудование = Обр;
	Зап.ОписаниеПараметров = ОписаниеПараметров;
	Зап.НаименованиеДляРеестра = НаименованиеДляРеестра;
...
Процедура тблОбрЦенаПриИзменении(Элемент)
	...
	тблОбрЦенаПриИзмененииНаСервере(Контрагент,текСтр.Оборудование,текСтр.Цена,текСтр.ОписаниеПараметров,текСтр.НаименованиеДляРеестра);
	ВидимостьКрасныхЦен();
КонецПроцедуры
...
Процедура тблОбрВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.имя <> "тблОбрЦена" и Поле.имя <> "тблОбрОписаниеПараметров" и Поле.имя <> "тблОбрНаименованиеДляРеестра" Тогда
		СтандартнаяОбработка = Ложь;
