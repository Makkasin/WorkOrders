...
Функция Данные(сс)
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   //|	ISNULL(СпрИмяКАНом.НаименованиеКА,  ISNULL(СпрИмяКА.НаименованиеКА+"" "","""")+СпрОбр.Наименование) КАК Оборудование,
	               |	CASE WHEN ро_РеестрыЗаказНаряды.НаименованиеДляРеестра="""" THEN СпрОбр.Наименование ELSE ро_РеестрыЗаказНаряды.НаименованиеДляРеестра END   КАК Оборудование,
...
Функция ДанныеРаботы(сс)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ДокЗН.Оборудование.Наименование КАК ОборудованиеЗН,   //Для сортировки
	               ...
	               |	CASE WHEN ро_РеестрыЗаказНаряды.НаименованиеДляРеестра="""" THEN ДокЗН.Оборудование.Наименование ELSE ро_РеестрыЗаказНаряды.НаименованиеДляРеестра END   КАК РаботаПоПрайсу,
	               |	неопределено КАК Работа,
	               |	ДокЗН.Оборудование.ЕдиницаИзмерения КАК ЕдИзм,
				   //|	ISNULL(СпрИмяКАНом.НаименованиеКА,ДокЗН.Оборудование.Наименование) КАК Оборудование,
	               |	CASE WHEN ро_РеестрыЗаказНаряды.НаименованиеДляРеестра="""" THEN ДокЗН.Оборудование.Наименование ELSE ро_РеестрыЗаказНаряды.НаименованиеДляРеестра END   КАК Оборудование,
	               |	ДокЗН.Оборудование.Наименование КАК ОборудованиеЗН,
	               |	ДокЗН.Оборудование.Диаметр КАК Диаметр,
	               |	ДокЗН.Оборудование.Давление КАК Давление,
	               |	CASE WHEN ДокЗН.НомерОборудованияКор="""" THEN ДокЗН.НомерОборудования ELSE ДокЗН.НомерОборудованияКор END КАК НомерОборудования,
	               |	CASE WHEN ДокЗН.ИнвНомерКор="""" THEN ДокЗН.ИнвНомер ELSE ДокЗН.ИнвНомерКор END КАК ИнвНомер,
...
Функция ПечатьРеестрСвод(сс,ссКА,ИмяКодМакета) Экспорт
	...
	Для каждого эл из МасКол Цикл
		Если нрег(Эл)="кол" или нрег(Эл)="ном" или нрег(Эл)="сумма" или лев(эл,1)="_"  Тогда  Продолжить; КонецЕсли;
		СтрСвр = СтрСвр+","+Эл;
	...
	Если  ЕстьНомОбр=Истина Тогда
		...
		надоСкобки = МасКол.Найти("_скобки")<>Неопределено;
		...
		Для каждого С из Т Цикл
			...
			Если надоСкобки  Тогда
				Если пСтр<>"" Тогда пСтр = "Зав.№ ("+пСтр+")"; КонецЕсли;
				Если пЗак<>"" Тогда пЗак = "Заказ-нар.№ ("+пЗак+")"; КонецЕсли;
			КонецЕСЛИ;
...
Функция ПечатьРеестрРабот(сс) Экспорт
	...
	Т = тбл.Скопировать();
	Т.свернуть("РаботаПоПрайсу,НомерОборудования,ИнвНомер,ЗаказНаряд,ОборудованиеЗН","Количество,Цена"); //теперь команда - сворачивать по заказ-наряду
	Т.Сортировать("ОборудованиеЗН,НомерОборудования,ИнвНомер,РаботаПоПрайсу");
	...
	Для каждого С из Т Цикл
		...
		Мас = Тбл.НайтиСтроки(Новый Структура("ЗаказНаряд",с.ЗаказНаряд));
		сткНом.номС=0;
		Для каждого Стр из Мас Цикл
