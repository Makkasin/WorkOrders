...
Процедура ДвиженияРабота(Отказ)
	...
	Для каждого Стр из ЗаказНаряды Цикл
		...
		рееЗап = Движения.ЗаказВРеестре.Добавить();
		рееЗап.ЗаказНаряд = Стр.ЗаказНаряд;
		рееЗап.ппНомерОборудования = Стр.ппНомерОборудования;
		рееЗап.Реестр = ссылка;
		
	КонецЦикла;
	
	Движения.РаботаРО.Записать();
	Движения.ЗаказВРеестре.Записать();
