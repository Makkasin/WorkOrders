...
Функция ЗаполнитьНаСервере(ДТ,КА,ФлтОбр,ЗкНаряд=Неопределено)
	...
	Запрос.Текст = "
				   ...
				   |INTO врЦенаЗН
				   |FROM врЦенаЗН0 Тбл
				   |GROUP BY Тбл.ЗаказНаряд,Тбл.ОписаниеПараметров
				   |;
				   ...
				   |    MAX(ISNULL(ЦенаЗН.Цена,ISNULL(Рег.цена,0)))*индОплаты Цена,
				   |    ISNULL(ЦенаЗН.ОписаниеПараметров,Рег.ОписаниеПараметров) ОписаниеПараметров
				   |
				   |
				   |
				   |
				   ...
				   |    ISNULL(ЦенаЗН.ОписаниеПараметров,Рег.ОписаниеПараметров)
				   |
...
Процедура ОбновитьРеквизитыОборудования(МасСтр = Неопределено,ОбновитьОписаниеПараметров=Ложь)
	...
	//Если ОбновитьОписаниеПараметров=Истина Тогда
	//	
	//	МасСтр.Сортировать("ГруппаРодитель,Диаметр,Давление");	
	//	пЦен   = Неопределено;
	//	пГрп   = Неопределено;
	//	текСтк = Неопределено;
	//	масСтк = новый Массив;
	//	
	//	Для а=1 по масстр.Количество() Цикл
	//		Стр = масСтр[а-1];
	//	
	//		Если пГрп<>Стр.ГруппаРодитель Тогда
	//			пГрп = стр.ГруппаРодитель;
	//			пЦен = Неопределено;
	//		КонецЕСЛИ;
	//		
	//		Если пЦен <> Стр.Цена Тогда
	//			
	...
	//				текСтк.Вставить("конСтр",а-2);
	//				
	//				масСтк.Добавить(текСтк);
	//			КонецесЛИ;
	//			
	//			пЦен = Стр.Цена;
	//			текСтк = Новый Структура("НачДиа,начДав,НачСтр",Стр.Диаметр,Стр.Давление,а-1);	
	//			
	//		КонецесЛИ;
	//		
	//	КонецЦикла;
	...
	//	КонецесЛИ;
	//	
	//	Для каждого Стк из масСтк Цикл
	//		Для а=Стк.начСтр по Стк.конСтр Цикл
	//			Диаметр  	= ФРМДиаметрДавление("Dy =",Стк.начДиа,Стк.конДИа); 
	//			Давление 	= ФРМДиаметрДавление("Py =",Стк.начДав,Стк.конДав); 
	//			
	//			масСтр[а].ОписаниеПараметров =  Диаметр+" "+Давление;
	//		Конеццикла;
	//	Конеццикла;
	//	
	//КонецЕслИ;
...
Процедура ЗаказНарядыЗаказНарядПриИзменении(Элемент)
	...
	//Работы
	пМас = ОБъект.Работы.НайтиСтроки("ЗаказНаряд",пСтр.ЗаказНаряд);
