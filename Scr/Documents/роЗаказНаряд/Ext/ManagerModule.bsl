...
Функция ДанныеДляПечати(ссМас) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	СпрОбр.Наименование КАК Оборудование,
	               |    CASE WHEN роЗаказНаряд.НомерОборудованияКор="""" THEN роЗаказНаряд.НомерОборудования ELSE роЗаказНаряд.НомерОборудованияКор END КАК НомерОборудования,
	...
	ЗаполнитьДефектыЗН(Стк,ссМас);
	ЗаполнитьПоказателиИспытаний(Стк,ссМас);
...
Процедура ЗаполнитьДефекты(Стк,ссМас)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	CASE WHEN ро_ОборудованиеДефектовка.Годен THEN ""Годен""  ELSE ""Не годен"" END КАК Годен,
...
Процедура ЗаполнитьДефектыЗН(Стк,ссМас)
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	роЗаказНаряд.Деталь.Наименование КАК Деталь,
	               |	роЗаказНаряд.Дефект КАК Дефект,
	               |	CASE WHEN роЗаказНаряд.Годен THEN ""Годен""  ELSE ""Не годен"" END КАК Годен,
	               |	роЗаказНаряд.Количество КАК ДетальКоличество
	               |ИЗ
	               |	Документ.роЗаказНаряд.Дефектовка КАК роЗаказНаряд
				   |ГДЕ роЗаказНаряд.Ссылка = &Ссылка
				   |
				   |";
	...
	Если Тбл.Количество() = 0 Тогда
		Стк.вставить("ДетальЗН1",Стк.Оборудование);
		Стк.вставить("ДефектЗН1","Дефектов не обнаружено");
		Стк.вставить("ГоденЗН1","Годен");
	ИНаче
		Для а=1 по Тбл.Количество() Цикл
			текСтр = Тбл[а-1];
			Для каждого Кол из Тбл.Колонки Цикл
				Стк.Вставить(""+Кол.имя+"ЗН"+а,текСтр[Кол.имя]);		
			КонецЦикла;
		КонецЦикла;
	КонецЕСлИ;
...
Процедура ЗаполнитьПоказателиИспытаний(Стк,ссМас)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	Показатель4
	               |ИЗ
	               |	Документ.роЗаказНаряд.Показатели КАК роЗаказНаряд
				   |ГДЕ роЗаказНаряд.Ссылка = &Ссылка
				   |
				   |";
	...
	Для а=1 по Тбл.Количество() Цикл
		текСтр = Тбл[а-1];
		Для каждого Кол из Тбл.Колонки Цикл
			Стк.Вставить(""+Кол.имя+"_"+а,текСтр[Кол.имя]);		
		КонецЦикла;
	КонецЦикла;
...
Функция ПечатьНакладная(Мас,Макет=Неопределено,ссПечФормы=Неопределено) Экспорт
	
	
	Если Макет = Неопределено Тогда
		Макет = Документы.роЗаказНаряд.ПолучитьМакет("АктИспытаний");
	КонецЕСЛИ;
	
	СткШапка = ДанныеДляПечати(Мас);
	Таб = Новый ТабличныйДокумент;
...
Функция ДанныеДефектовка(сс)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	роЗаказНаряд.Контрагент КАК КонтрагентСсылка,
				   ...
				   |    CASE WHEN роЗаказНаряд.НомерОборудованияКор="""" THEN роЗаказНаряд.НомерОборудования ELSE роЗаказНаряд.НомерОборудованияКор END  НомерОборудования,
	               ...
	               |	роЗаказНаряд.КонтролерДеф КАК КонтролерДеф,
	               |	роЗаказНаряд.КонтролерДеф.Должность КАК длжКонтролерДеф,
	...
	ТБл = Запрос.Выполнить().Выгрузить();
	Если Тбл.Количество()=0 ТОгда ТБл.Добавить(); КонецЕСлИ;
	...
	ДЛя каждого Стр из ТБл Цикл
		...
		Конеццикла;
		Если ЗначениеЗаполнено(Стк.КонтролерДеф)=Ложь Тогда
			Стк.КонтролерДеф    = Стк.КонтролерСТК;
			Стк.длжКонтролерДеф = Стк.длжКонтролерСТК;
		КонецеСЛИ;
		
		Стк.вставить("КонтролерДеф",СПравочники.ФизическиеЛица.ФИОстр(Стк.КонтролерДеф));
...
Функция ПечатьДефектовка(сс,СткДопИнфо=Неопределено) Экспорт
	...
	Макет = Справочники.ПечатныеФормы.НайтиПечФормуПоВиду(Перечисления.ВидыДокументовДЛяПечФорм.ДефВедомость,сткШапка.КонтрагентСсылка);
	Если Макет =Неопределено Тогда
	...
	СткШАпка.Тбл.Колонки.Добавить("Ном",Новый ОписаниеТипов("Число"));
	ном = 0;
