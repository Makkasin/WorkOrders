...
Процедура КЭШ(соо)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	роЗаказНаряд.Статус в (Значение(Справочник.ро_Статусы.ВРаботе),Значение(Справочник.ро_Статусы.ОчередьСТК))
...
&НаСервереБезКонтекста
Функция ЗаполнитьИзСтатусаНаСервере()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	И роЗаказНаряд.Статус = Значение(Справочник.ро_Статусы.ОчередьСТК)
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	роЗаказНаряд.Номер";
...
&НаКлиенте
Процедура ЗаполнитьИзСтатуса(Команда)
	мас = ЗаполнитьИзСтатусаНаСервере();
	Для каждого эл из Мас Цикл
		м= тблОборудование.НайтиСтроки(Новый Структура("ЗаказНаряд",Эл));
		Если м.Количество()<>0 Тогда Продолжить; КонецЕсли;
		
		новСтр = тблОборудование.Добавить();
		новСТр.ЗаказНаряд = эл;
