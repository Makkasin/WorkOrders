...
Процедура ДобавитьКнопкиПечати(Форма,элементСпрСС,ВидДокумента,ФильтрКонтрагент=Неопределено) Экспорт
		...
		Если тблПеч.Количество()>3 и Элементы.ГруппаПечатныеКоманды.Вид = ВидГруппыФормы.ГруппаКнопок Тогда
...
Функция ДанныеДляПечати(ссМас,МасКол,сткШапка,ссПечФормы=Неопределено) Экспорт
	...
	ТекстЗапросаСсПечФормы = Неопределено;
	Если ссПечФормы<>Неопределено Тогда
		...
		Если СокрЛП(ссПечФормы.ТекстЗапроса)<>"" Тогда
			  ТекстЗапросаСсПечФормы =  ссПечФормы.ТекстЗапроса;
			  Запрос.Текст =  СтрЗаменить(нрег(Запрос.Текст),"выбрать","выбрать TOP 1");
		КонецЕСЛИ;
	КонецЕсли;
	...
	Для каждого Стр из Тбл Цикл
		...
		сткШапка.вставить("длжКонтролерСТК",Стр.длжКонтролерСТК);
		...
		сткШапка.вставить("ДатаОтгрузка",Формат(Стр.ДатаОтг,"ДФ=dd.MM.yyyy"));
		сткШапка.вставить("ДатаЗаказа",Формат(Стр.ДатаЗаказа0,"ДФ=dd.MM.yyyy"));
	...
	Если ТекстЗапросаСсПечФормы<>Неопределено Тогда
		Запрос.Текст = ТекстЗапросаСсПечФормы;
		Тбл = Запрос.Выполнить().Выгрузить();
		
		Если Тбл.Колонки.НАйти("Количество")=Неопределено Тогда
			ТБл.Колонки.Добавить("Количество");
		КонецеСЛИ;
		Если Тбл.Колонки.НАйти("КоличествоМеталлолом")=Неопределено Тогда
			ТБл.Колонки.Добавить("КоличествоМеталлолом");
		КонецеСЛИ;
		Если Тбл.Колонки.НАйти("Сумма")=Неопределено Тогда
...
Функция ЗаполнитьСткШапкаСдалПринял(сткШапка,ЭтоПриход)
	Если ЭтоПриход ТОгда 
		...
		сткШапка.вставить("ДатаСдалПринял",сткШапка.ДатаПриход);
	ИНаче
		...
		сткШапка.вставить("ДатаСдалПринял",сткШапка.ДатаОтгрузка);
	КонецЕсли;
КонецФункции
